...
public class JoinNSD extends AppCompatActivity implements PeerListFragment.OnListFragmentInteractionListener {
    ...
    private void initialize() {
        ...
        String myIP = Utility.getWiFiIPAddress(JoinNSD.this);
        Utility.saveString(JoinNSD.this, TransferConstants.KEY_MY_IP, myIP);
...
//        connListener.start();
    ...
    protected void onResume() {
        ...
        LocalBroadcastManager.getInstance(JoinNSD.this).registerReceiver(localDashReceiver,
                filter);
...
//        mNsdHelper.registerService(ConnectionUtils.getPort(LocalDashNSD.this));
    ...
    protected void onDestroy() {
        //mNsdHelper.tearDown();
        Utility.clearPreferences(JoinNSD.this);
        appController.stopConnectionListener();
        mNsdHelper.tearDown();
        mNsdHelper = null;
        DBAdapter.getInstance(JoinNSD.this).clearDatabase();
        super.onDestroy();
    }
...
//                    DataSender.sendCurrentDeviceData(LocalDashNSD.this, ip, port, true);
                    ...
                    DataSender.sendCurrentDeviceData(JoinNSD.this, ipAddress, port, true);
                    break;
                case DataHandler.DEVICE_LIST_CHANGED:
                    ArrayList<DeviceDTO> devices = DBAdapter.getInstance(JoinNSD.this)
                            .getDeviceList();
...
//                    DialogUtils.getChatRequestDialog(JoinNSD.this, chatRequesterDevice).show();
//                    break;
                case DataHandler.CHAT_RESPONSE_RECEIVED:
                    ...
                    if (!isChatRequestAccepted) {
                        NotificationToast.showToast(JoinNSD.this, "Chat request " +
                                "rejected");
                    } else {
                        DeviceDTO chatDevice = (DeviceDTO) intent.getSerializableExtra(DataHandler
                                .KEY_CHAT_REQUEST);

                        // Não muda activity ao ser aceito no grupo
//                        DialogUtils.openChatActivity(JoinNSD.this, chatDevice);
                        NotificationToast.showToast(JoinNSD.this, chatDevice
        ...
        switch (requestCode) {
            case DialogUtils.CODE_PICK_IMAGE:
                if (resultCode == RESULT_OK) {
                    ...
                    DataSender.sendFile(JoinNSD.this, selectedDevice.getIp(),
    ...
    public void onListFragmentInteraction(DeviceDTO deviceDTO) {
        NotificationToast.showToast(JoinNSD.this, deviceDTO.getDeviceName() + " clicked");
        selectedDevice = deviceDTO;
//        showServiceSelectionDialog();
        DialogUtils.getServiceSelectionDialog(JoinNSD.this, deviceDTO);
    }
}